...
namespace DispatcherSwitchIconDatabase
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Run8 DispatcherSwitchIconDatabase Parser");
            ...
            using (FileStream fileStream = new FileStream(path, FileMode.Open))
            {
                using (BinaryReader binaryReader = new BinaryReader(fileStream))
                {
                    ...
                    for (int i = 0; i < num0; i++)
                    {
                        ...
                        Console.WriteLine("     DispatcherSwitchIcon {0}: num1={1}, recX={2}, recY={3}, recW={4}, recH={5}, vec2X={6}, vec2Y={7}, int0={8}, int1={9}", i, num1, recX, recY, recW, recH, vec2X, vec2Y, int0, int1);

                        int num2 = binaryReader.ReadInt32(); // entry - number of unknown1

                        Console.WriteLine("     DispatcherSwitchIcon {0}: Number of unknown1 {1}", i, num2);

                        for (int j = 0; j < num2; j++)
                        {
                            int int2 = binaryReader.ReadInt32(); // unknown1 - unknown
                            Console.WriteLine("         DispatcherSwitchIcon {0} Unknown1 {1}: int2={2}", i, j, int2);
                        }

                        if (num1 == 2)
                        {
                            string string0 = ReadString(binaryReader); // unknown2 - unknown
                            Console.WriteLine("         DispatcherSwitchIcon {0} Unknown2: string0={2}", i, string0);
                        }
                    }
                }
            }
