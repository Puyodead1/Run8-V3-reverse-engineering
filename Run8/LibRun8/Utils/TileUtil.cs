    ...
    public class TileUtil
    {
        public static void smethod_13(TerrainTile tile)
        {
            ...
            for (int i = 0; i < Chunk.CHUNK_SIZE; i++)
            {
                ...
                {
                    int num4 = 0;
                    tile.chunkData[i, j].vertices = new VertexStruct[(tile.chunkData[i, j].hixels * tile.chunkData[i, j].hixels)];
                    int count = indices.Count;
                    for (int k = 0; k < tile.chunkData[i, j].hixels; k++)
                    {
                        for (int l = 0; l < tile.chunkData[i, j].hixels; l++)
                        {
                            VertexStruct vertex = new VertexStruct();
                            Vector3 position = vertex.Position;
                            Vector2 uvCoords = vertex.TextureCoordinate;

                            position.X = i * num + num / (tile.chunkData[i, j].hixels - 1) * k;
                            position.Z = -(j * num2 + num2 / (tile.chunkData[i, j].hixels - 1) * l);
                            position.Y = tile.chunkData[i, j].heightMap[k, l];
                            ...
                            tile.chunkData[i, j].vertices[num4] = vertex;
                            vertCount++;
                            num4++;
                        }
                    }
                    SetUpChunkIndices(tile.chunkData[i, j].hixels, vertCount, indices);
                    CalculateNormals(tile.chunkData[i, j].vertices);
                }
            }
